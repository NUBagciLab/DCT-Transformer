...
class EncoderBlock(nn.Module):
    ...
    def __init__(
        ...
        self.self_attention = nn.MultiheadAttention(round(hidden_dim*dct_ratio), num_heads, dropout=attention_dropout, batch_first=True)
        ...
        self.dct_weight = nn.Parameter(torch.Tensor(dct(np.eye(hidden_dim), axis=-1, norm="ortho")[:, :round(hidden_dim*dct_ratio)]), requires_grad = False)
        self.idct_weight = nn.Parameter(torch.Tensor(idct(np.eye(hidden_dim), axis=-1, norm="ortho")[:round(hidden_dim*dct_ratio), :]), requires_grad = False)
