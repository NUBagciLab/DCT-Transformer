...
def shifted_window_attention(
    ...
    qkv = F.linear(x, torch.cat([q_weight, k_weight, v_weight]), qkv_bias)
...
class ShiftedWindowAttention(nn.Module):
    ...
    def __init__(
        ...
        self.q_weight = nn.Parameter(torch.Tensor(dct(np.eye(dim), axis=-1, norm="ortho").T), requires_grad = False)
        self.k_weight = nn.Parameter(torch.Tensor(dct(np.eye(dim), axis=-1, norm="ortho").T), requires_grad = False)
        self.v_weight = nn.Parameter(torch.empty((dim, dim)))
        init.kaiming_uniform_(self.v_weight, a=math.sqrt(5))
        if qkv_bias:
            self.qkv_bias = nn.Parameter(torch.empty(dim * 3))
            fan_in, _ = init._calculate_fan_in_and_fan_out(torch.cat([self.q_weight, self.k_weight, self.v_weight]))
            bound = 1 / math.sqrt(fan_in) if fan_in > 0 else 0
            init.uniform_(self.qkv_bias, -bound, bound)
        else:
