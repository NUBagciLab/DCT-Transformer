...
def shifted_window_attention(
    ...
    x_dct = dct(x)[..., :round(C*dct_ratio)]
    qkv = F.linear(x_dct, qkv_weight, qkv_bias)
    qkv = qkv.reshape(x.size(0), x.size(1), 3, num_heads, round(C*dct_ratio) // num_heads).permute(2, 0, 3, 1, 4)
    ...
    x_dct = attn.matmul(v).transpose(1, 2).reshape(x.size(0), x.size(1), round(C*dct_ratio))
    x = torch.nn.ConstantPad1d((0, C-round(C*dct_ratio)), 0)(x_dct)
...
class ShiftedWindowAttention(nn.Module):
    ...
    def __init__(
        ...
        self.qkv = nn.Linear(round(dim*dct_ratio), round(dim*dct_ratio) * 3, bias=qkv_bias)
