...
class ShiftedWindowAttention(nn.Module):
    ...
    def __init__(
        ...
        self.qkv.weight.data[dim : dim * 2] =  torch.from_numpy(dct(np.eye(dim), axis=-1, norm="ortho").T)
        self.qkv.weight.data[dim * 2:] =  torch.from_numpy(dct(np.eye(dim), axis=-1, norm="ortho").T)
