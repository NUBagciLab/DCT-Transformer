...
def shifted_window_attention(
    ...
    dct_x = F.linear(x, dct_weight)
    qkv = torch.cat([F.linear(x, q_weight), dct_x, dct_x], dim=-1) + qkv_bias
...
class ShiftedWindowAttention(nn.Module):
    ...
    def __init__(
        ...
        self.dct_weight = nn.Parameter(torch.Tensor(dct(np.eye(dim), axis=-1, norm="ortho").T), requires_grad = False)
        if qkv_bias:
            self.qkv_bias = nn.Parameter(torch.empty(dim * 3))
            fan_in, _ = init._calculate_fan_in_and_fan_out(self.q_weight)
